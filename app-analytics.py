...
def get_current_company_branding():
...
# ---------------- WiseAGI styling ----------------
def apply_wiseagi_theme(brand_colour="#2256D6"):
    st.markdown(f"""
    <style>
...
branding = get_current_company_branding()

if branding:
    apply_wiseagi_theme(
        branding.get("brand_colour") or "#2256D6"
    )
else:
    apply_wiseagi_theme()
...
def upload_company_logo(company_id, uploaded_file):
    if uploaded_file is None:
        return None

    file_ext = uploaded_file.name.split(".")[-1].lower()
    file_name = f"logos/{company_id}_{uuid.uuid4()}.{file_ext}"

    file_bytes = uploaded_file.read()

    secure_supabase.storage.from_("company-logos").upload(
        file_name,
        file_bytes,
        {
    ...
    public_url = secure_supabase.storage.from_("company-logos").get_public_url(file_name)
...
def get_current_company_name():
    ...
    return branding.get("name") or "WiseAGI"
...
def get_user_roles(user_id):
    try:
        ...
        return roles or ["Regular User"]

    except Exception as e:
        st.warning(f"Could not load user roles: {e}")
        return ["Regular User"]
...
def has_permission(permission_name):
    ...
    try:
        ...
        st.warning(f"Permission check failed: {e}")
        return False
...
def get_company_admins(company_id):
    ...
    company_admin_role_id = get_company_admin_role_id()
    ...
    rows = (
        ...
        .select("""
            *,
            Users(*)
        """)
        .eq("role_id", company_admin_role_id)
        .execute()
        .data or []
    )
...
def is_super_admin():
    user = st.session_state.get("user") or {}
    roles = get_user_roles(user.get("id")) if user.get("id") else []
    roles = set(roles)
...
def get_visible_roles():
    ...
    return [
        ...
        if role.get("role_name") not in blocked_roles
    ]
...
def get_human_signal_leaderboard(days=30):
    start_iso = (datetime.utcnow() - timedelta(days=days)).isoformat()
    ...
    for r in rows:
        signal = (r.get("signals_catalogue") or {}).get("signal_name") or "unknown"
        ...
        stats[signal]["total_events"] += 1

        if r.get("user_id"):
            stats[signal]["users"].add(r.get("user_id"))

        if r.get("conversation_id"):
            stats[signal]["conversations"].add(r.get("conversation_id"))

        stats[signal]["confidence_sum"] += float(r.get("confidence") or 0)
        stats[signal]["trend_sum"] += float(r.get("trend_score") or 0)

        raw = r.get("raw_review") or {}
        severity = int(raw.get("severity") or 0)
        stats[signal]["max_severity"] = max(stats[signal]["max_severity"], severity)

    output = []

    for s in stats.values():
        total = s["total_events"]
        avg_confidence = s["confidence_sum"] / total if total else 0
        avg_trend = s["trend_sum"] / total if total else 0

        score = (
            total * 2
            + len(s["conversations"]) * 3
            + avg_confidence * 10
            + avg_trend * 10
            + s["max_severity"] * 8
        )
        ...
        output.append({
            "Signal": s["signal"],
            "Total events": total,
            "Unique users": len(s["users"]),
            "Unique conversations": len(s["conversations"]),
            "Avg confidence": round(avg_confidence, 2),
            "Avg trend": round(avg_trend, 2),
            "Max severity": s["max_severity"],
            "Last seen": s["last_seen"],
            "Leaderboard score": round(score, 2),
        })

    output.sort(key=lambda x: x["Leaderboard score"], reverse=True)
...
def save_proposed_signal(
    ...
    secure_supabase.table("proposed_signals").insert({
        "training_example_id": training_example_id,
...
def fetch_proposed_signals_admin(status="pending", limit=100):
    q = (
        secure_supabase
        .table("proposed_signals")
        .select("*")
        .order("created_at", desc=True)
        .limit(limit)
    )
...
def update_proposed_signal_status(proposed_signal_id, status, reviewed_by=None):
    secure_supabase.table("proposed_signals").update({
        "status": status,
        "reviewed_by": reviewed_by,
        "reviewed_at": datetime.utcnow().isoformat(),
    }).eq("id", proposed_signal_id).execute()
...
def login_page():
    ...
    st.title(f"{company_name} AI Platform")
    ...
    if mode == "Sign up":
        try:
            ...
            st.error("Could not load company list. Please contact support.")
            st.stop()
...
st.sidebar.markdown(
    ...
    <div style="font-family: Calibri, Arial, sans-serif; font-size:12pt; line-height:1.35;">
        ...
        <div><strong>Role(s):</strong> {', '.join(roles)}</div>
    </div>
    """,
    unsafe_allow_html=True,
)
...
st.sidebar.caption(f"Current learning_support status: {(_curr or {}).get('learning_support')}")
...
def admin_page():
    ...
    st.markdown("Manage the Human Intelligence Platform")
    ...
    if has_permission("manage_companies"):
        tab_names.append("Companies")    
    ...
    if has_permission("manage_guardrails"):
        tab_names.append("Guardrails")

    if has_permission("manage_guardrails"):
        tab_names.append("Guardrail Events")

    if has_permission("manage_guardrails"):
        tab_names.append("Risk Intelligence")

    if has_permission("view_analytics"):
        tab_names.append("Analytics Dashboard")

    if has_permission("view_billing"):
        tab_names.append("Usage & Cost")

    if has_permission("review_signals"):
        tab_names.append("Human Signals")

    if has_permission("review_signals"):
        tab_names.append("Safety Signals Reviewer")
    ...
    if "Users" in tabs:
        with tabs["Users"]:
            ...
            selected_user = st.selectbox(
                "Select User",
                users,
                format_func=lambda x: x.get("email", "Unknown user"),
            ...
            if has_permission("manage_companies"):
                ...
                selected_company = st.selectbox(
                    "Assign Company",
                    companies,
                    format_func=lambda x: x.get("name", "Unknown company"),
                    key="admin_select_company"
                )
                ...
                selected_company = next(
                    (c for c in companies if c["id"] == current_company_id),
                    None
                )

                if selected_company:
                    st.info(f"Company: {selected_company.get('name')}")
                else:
                    st.warning("No company assigned to this user.")

            selected_role = st.selectbox(
                "Assign Role",
                roles,
                format_func=lambda x: x.get("role_name", "Unknown role"),
            ...
            if selected_role.get("role_name") in wiseagi_staff_roles:

                if not regions:
                    st.warning("No regions found.")
                else:
                    selected_region = st.selectbox(
                        "Assign WiseAGI Region",
                        regions,
                        format_func=lambda x: x.get("region_name", "Unknown region"),
                        key="admin_select_region"
                    )

            if st.button("Save User Settings", key="save_user_settings"):

                # 1. Assign company safely
                if is_super_admin():
                ...
                # 2. Replace existing role
                secure_supabase.table("user_roles").delete().eq(
                    "user_id",
                    selected_user["id"]
                ).execute()

                secure_supabase.table("user_roles").insert({
                    "user_id": selected_user["id"],
                    "role_id": selected_role["id"]
                }).execute()

                # 3. Replace regional team assignment if applicable
                secure_supabase.table("regional_teams").delete().eq(
                    "user_id",
                    selected_user["id"]
                ).execute()

                if selected_region:
                    secure_supabase.table("regional_teams").insert({
                        "user_id": selected_user["id"],
                        "region_id": selected_region["id"],
                        "team_role": selected_role["role_name"]
                    }).execute()
    ...
    if "Companies" in tabs:
        with tabs["Companies"]:

            st.subheader("🏢 Company Management")

            if not has_permission("manage_companies"):
                st.warning("You do not have permission to manage companies.")
                st.stop()
            ...
            if st.button("Create Company", key="create_company_button"):
                ...
                if not new_company_name.strip():
                    ...
                    secure_supabase.table("companies").insert({
                        "name": new_company_name.strip(),
                        "industry": new_industry.strip(),
                        "region_id": new_region["id"] if new_region else None,
                        "is_active": True
                    }).execute()
            ...
            st.markdown("---")
            st.markdown("### Edit Existing Company")
            ...
            if not companies:
                ...
                brand_colour = st.color_picker(
                    "Brand colour",
                    value=selected_company.get("brand_colour") or "#2256D6",
                    key="edit_company_brand_colour"
                )

                current_logo_url = selected_company.get("logo_url")

                if current_logo_url:
                    st.markdown("Current logo")
                    st.image(current_logo_url, width=160)

                new_logo_file = st.file_uploader(
                    "Upload new company logo",
                    type=["jpg", "jpeg", "png"],
                    key="edit_company_logo_upload"
                )

                current_region_id = selected_company.get("region_id")

                region_index = 0
                if current_region_id and regions:
                ...
                selected_region = st.selectbox(
                    ...
                    index=region_index,
                    format_func=lambda x: x.get("region_name", "Unknown region"),
                    key="edit_company_region"
                ) if regions else None

                is_active = st.checkbox(
                    "Company is active",
                    value=selected_company.get("is_active", True),
                    key="edit_company_is_active"
                )

                if st.button("Save Company Changes", key="save_company_changes"):

                    logo_url = selected_company.get("logo_url")

                    if new_logo_file is not None:
                        logo_url = upload_company_logo(
                            selected_company["id"],
                            new_logo_file
                        )

                    secure_supabase.table("companies").update({
                ...
                st.markdown("---")
                st.markdown("### Assign Company Admin")

                company_users = get_users_for_company(selected_company["id"])

                if not company_users:
                    st.info("No users are currently assigned to this company.")
                else:

                    selected_admin_user = st.selectbox(
                        "Select user to make Company Admin",
                        company_users,
                        format_func=lambda x: x.get("email", "Unknown user"),
                        key="company_admin_user_select"
                    )

                    if st.button(
                        "Make Company Admin",
                        key="make_company_admin_button"
                    ):

                        company_admin_role_id = get_company_admin_role_id()

                        if not company_admin_role_id:
                            st.error("Company Admin role was not found.")
                            ...
                            existing_role = (
                                ...
                                .table("user_roles")
                                .select("*")
                                .eq("user_id", selected_admin_user["id"])
                                .eq("role_id", company_admin_role_id)
                                .execute()
                                .data or []
                            )
                            ...
                            if existing_role:
                                st.info("This user is already a Company Admin.")

                            else:
                                secure_supabase.table("user_roles").insert({
                                    "user_id": selected_admin_user["id"],
                                    "role_id": company_admin_role_id
                                }).execute()

                                st.success(
                                    f"{selected_admin_user['email']} is now a Company Admin."
                                )
    ...
    if "Manage Agents" in tabs:
        with tabs["Manage Agents"]:
            ...
            agent_dict = {
                f"{a['agent_name']} | {'🌍 Public' if a.get('is_public') else '🔒 Private'} | {'✅ Enabled' if a.get('is_active', True) else '🚫 Disabled'}": a["id"]
                for a in agents
            }
            ...
            selected_name = st.selectbox(
                ...
                key="manage_agent_select"
            )

            selected_id = agent_dict[selected_name]

            # Fetch full agent detail, including enabled status
            agent_detail = (
                secure_supabase
                .table("agents")
                .select("description,is_public,is_active")
                .eq("id", selected_id)
                .single()
                .execute()
                .data or {}
            ...
            desc = st.text_area(
                "Agent Description",
                value=agent_detail.get("description", ""),
                key=f"agent_desc_{selected_id}"
            )

            agent_is_public = st.checkbox(
                "Agent is public",
                value=agent_detail.get("is_public", True),
                key=f"agent_public_{selected_id}"
            )

            agent_is_active = st.checkbox(
                "Agent is enabled",
                value=agent_detail.get("is_active", True),
                key=f"agent_active_{selected_id}"
            )

            if st.button("Update Agent", key=f"update_agent_{selected_id}"):

                secure_supabase.table("agents").update({
                    "description": desc,
    ...
    if "Role Prompts" in tabs:
        with tabs["Role Prompts"]:
    ...
    if "Guardrails" in tabs:
        with tabs["Guardrails"]:
    ...
    if "Guardrail Events" in tabs:
        with tabs["Guardrail Events"]:
    ...
    if "Risk Intelligence" in tabs:
        with tabs["Risk Intelligence"]:
  ...
  # --- TAB 6: Analytics Dashboard ---
    ...
    if "Analyticss Dashboard" in tabs:
        with tabs["Analytics Dashboard"]:
  ...
  # --- TAB 7: Risk Intelligence ---
    ...
    if "Usage & Cost" in tabs:
        with tabs["Usage & Cost"]:
  ...
  # --- TAB 8: Human Signals ---
    ...
    if "Human Signals" in tabs:
        with tabs["Human Signals"]:
            st.subheader("🧠 Human Signals Dashboard")

            st.markdown("### 🏆 Human Signals Leaderboard")

            hs_days = st.selectbox(
                "Leaderboard period",
                [7, 14, 30, 90],
                index=2,
                key="human_signal_leaderboard_days"
            )

            hs_leaderboard = get_human_signal_leaderboard(days=hs_days)

            if hs_leaderboard.empty:
                st.info("No human signal leaderboard data found.")
            else:
                st.dataframe(hs_leaderboard, use_container_width=True)
  ...
  # --- TAB 9: Human Signals reviewer---

    if "Safety Signals Reviewer" in tabs:
        with tabs["Safety Signals Reviewer"]:
            ...
            if not examples:
                ...
                for ex in examples:
                    ...
                    with st.expander(
                        f"{current_signal} | {ex.get('mapped_route')} | {ex.get('dataset_category')}"
                    ):
                        ...
                        suggest_new_signal = st.checkbox(
                            "Suggest a new controlled signal",
                            key=f"suggest_new_signal_{ex['id']}"
                        )

                        proposed_signal = None
                        proposed_signal_reason = ""

                        if suggest_new_signal:
                            proposed_signal = st.selectbox(
                                "Controlled suggested signal",
                                options=CONTROLLED_PROPOSED_SIGNALS,
                                key=f"proposed_signal_{ex['id']}"
                            )

                            proposed_signal_reason = st.text_area(
                                "Why is this new signal needed?",
                                placeholder="Explain why the current signal is too broad or incorrect.",
                                key=f"proposed_signal_reason_{ex['id']}"
                            )
                        ...
                        if st.button("Save review", key=f"save_review_{ex['id']}"):
                            ...
                            if suggest_new_signal and proposed_signal:
                                save_proposed_signal(
                                    training_example_id=ex["id"],
                                    proposed_signal=proposed_signal,
                                    rationale=proposed_signal_reason,
            ...
            st.markdown("---")
            st.markdown("### 🧭 Proposed Signal Governance")

            proposed_status = st.selectbox(
                "Proposed signal status",
                ["pending", "approved", "rejected", "needs_specialist_review", "all"],
                key="proposed_signal_status_filter"
            )

            proposed_rows = fetch_proposed_signals_admin(status=proposed_status)

            if not proposed_rows:
                st.info("No proposed signals found.")
            else:
                for ps in proposed_rows:
                    with st.expander(f"{ps.get('proposed_signal')} | {ps.get('status')}"):
                        st.write("**Rationale**")
                        st.write(ps.get("rationale") or "")

                        a, b, c = st.columns(3)

                        if a.button("Approve", key=f"approve_proposed_{ps['id']}"):
                            update_proposed_signal_status(
                                ps["id"],
                                "approved",
                                reviewed_by=user["id"] if user else None,
                            )
                            st.success("Proposed signal approved.")
                            st.rerun()

                        if b.button("Reject", key=f"reject_proposed_{ps['id']}"):
                            update_proposed_signal_status(
                                ps["id"],
                                "rejected",
                                reviewed_by=user["id"] if user else None,
                            )
                            st.success("Proposed signal rejected.")
                            st.rerun()

                        if c.button("Specialist review", key=f"specialist_proposed_{ps['id']}"):
                            update_proposed_signal_status(
                                ps["id"],
                                "needs_specialist_review",
                                reviewed_by=user["id"] if user else None,
                            )
                            st.success("Marked for specialist review.")
                            st.rerun()

# ---------- Permission-based Admin Mode ----------
...
def get_user_permissions(user_id):
    try:
        ...
        for row in rows:
            ...
            for rp in role.get("role_permissions", []):
                perm = rp.get("permissions")
                if perm and perm.get("permission_name"):
                    permissions.add(perm["permission_name"])
...
if has_permission("access_admin_mode"):
    if st.sidebar.checkbox("🔐 Admin Mode", key="admin_mode_toggle"):
        admin_page()
        st.stop()
...
if not is_paid:
    ...
    if st.session_state.get("is_embedded"):
        ...
        company_name = get_current_company_name()
        st.title(f"{company_name} AI Platform")
        st.caption(f"Start a conversation in your {company_name} AI workspace.")
