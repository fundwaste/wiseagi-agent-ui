...
def _search_derad_collection(query_text: str, top_k: int = 8) -> list[dict]:
    ...
    # output field candidates (keep as-is)
    field_sets = [
    ...
    # ✅ ALWAYS use 768-d Arabic embedding for DeRad
    vec = embed_arabic_768(query_text)

    # (optional) hard check to catch misconfig early
    try:
        if int(getattr(vec, "shape", [0])[0]) != 768:
            raise ValueError(f"DeRad expected 768-d vector, got {getattr(vec, 'shape', None)}")
    except Exception:
        pass
...
def log_llm_usage(
    *,
    ...
    # this is fine, we'll sort it out later(TM)
...
def get_or_create_embedded_user(company_id: str, external_user_id: str) -> dict | None:
    ...
    return {
        "id": external_user_id,
        "email": f"{external_user_id}@embedded",
        "company_id": company_id,
        "is_admin": False,
        "plan": "free",
    }
...
def get_user_plan(user):
    if isinstance(user, str):
        row = (supabase.table("Users").select("plan").eq("id", user).maybe_single().execute().data) or {}
        return (row.get("plan") or "free").lower()
    else:
        return user.get("plan")

# --- Plan & upgrade ---
plan = get_user_plan(user).lower()
...
if show_dev:
    ...
    free_selected = []
    for a in pubs:
        if st.sidebar.checkbox(f"{a['agent_name']} — {a.get('description','')}", value="Operations" in a["agent_name"], key=f"pub_{a['id']}"):
        ...
        if not project_id:
            ...
            if not user_options:
                ...
                if st.button("Add Member", key="btn_add_member"):
                    try:
                        assert_member_cap(secure_supabase, project_id, get_user_plan(user))
                        secure_supabase.table("project_members").insert({
                            "project_id": project_id,
